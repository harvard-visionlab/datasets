...
| **fps stores** `…-{640x360,456x256}-{30,15}fps` (integer decimation, exact timestamps, `src_fps`/`src_num_frames` fields) | `shards/<res>-<fps>fps/` → `stores/`, S3 | **encode done 2026-09-17** on the 5-host fleet: 30 fps 74/74 groups (365,176 clips + 118 patched; 311 GB + 176 GB), 15 fps 74/74 (365,290 clips; 295 GB + 167 GB). Remaining: a handful of variable-frame-rate clips (2 at 30 fps, 6 at 15 fps) being re-encoded by the patch pass with the VFR fallback (`select` only, original timestamps); then `finish.py` (running on machina) merges each axis and syncs it to S3. **Check:** `fleet.py status --fps 30|15` (bottom line = patch coverage), `logs/finish.log`, `s5cmd ls s3://visionlab-datasets/slipstream-cache/spatialvid-hq/`. Lessons: exact rate / frame-count asserts must tolerate VFR sources; a patch shard with 0 records has a column-less parquet (guarded). |
