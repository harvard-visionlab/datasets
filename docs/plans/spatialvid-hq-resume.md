...
- Fleet containers: no GitHub ssh key (repo is public → https pull, or `fleet.py sync` rsyncs this checkout), `nbstripout` filter missing, `DataLocal` read-only (scratch = `/tmp`), QNAP Flash at `~/work/DataRemote/qnap/exactitude/Flash` (machina: `~/work/DataExactitudeFlash`).
