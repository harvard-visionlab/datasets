"""Upload ImageNet-100 / ImageNet-10 train caches to S3.
...
Usage::
    ...
    # Sync all known train caches that exist locally (default)
    uv run python scripts/sync_train_to_s3.py

    # Sync only specific caches by exact name
    uv run python scripts/sync_train_to_s3.py \\
        imagenet100-s292_l584-jpeg-train imagenet100-s292_l584-yuv420-train

    # Sync any cache whose name contains a substring (e.g. all s292)
    uv run python scripts/sync_train_to_s3.py --match s292

    # List what would be synced without uploading
    uv run python scripts/sync_train_to_s3.py --match s292 --dry-run
"""
import argparse
...
CACHES = [
    ...
    ("imagenet100-s292_l584-jpeg-train", "imagenet100"),
    ("imagenet100-s292_l584-yuv420-train", "imagenet100"),
