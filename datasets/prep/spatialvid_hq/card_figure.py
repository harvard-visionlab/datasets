...
def main():
    ...
    ax.grid(alpha=.3); ax.legend(fontsize=9, loc="center right"); ax.set_xlabel("t (s)")
    ax.set_title("ego-motion translation per frame (pose units), in the previous frame's axes\n(steps: source poses are ~5 Hz, interpolated linearly between keyframes)", fontsize=10)
