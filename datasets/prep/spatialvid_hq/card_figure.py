...
def main():
    ...
    c = camera_centers(poses[k]); tt = t[k]; show = np.linspace(0, T - 1, 6).astype(int)
    fig = plt.figure(figsize=(15, 8.2)); gs = fig.add_gridspec(3, 6, height_ratios=[1.35, 1.6, 1.0], hspace=0.5, wspace=0.08)
    for col, j in enumerate(show):
        ax = fig.add_subplot(gs[0, col]); ax.imshow(frames[k, j].permute(1, 2, 0).numpy()); ax.axis("off")
        ax.set_title(f"frame {j}   t = {tt[j] - tt[0]:.1f} s", fontsize=10)
    ax = fig.add_subplot(gs[1, :3])
    ax.plot(c[:, 0], c[:, 2], "-", color="0.6", lw=1); sc = ax.scatter(c[:, 0], c[:, 2], c=tt - tt[0], cmap="viridis", s=14, zorder=3)
    for j in show:
        ax.annotate(str(j), (c[j, 0], c[j, 2]), textcoords="offset points", xytext=(5, 5), fontsize=9)
    ax.set_aspect("equal"); ax.grid(alpha=.3); ax.set_xlabel("x (right)"); ax.set_ylabel("z (forward at t=0)")
    ax.set_title("camera position, top-down (poses interpolated to each of the 120 frames; units are per-clip, not metres)", fontsize=10)
    fig.colorbar(sc, ax=ax, label="t (s)", fraction=0.03, pad=0.02)
    ax = fig.add_subplot(gs[1, 3:])
    ax.plot(tt[1:] - tt[0], ego[k, :, 2], label="dz  (forward step per frame)"); ax.plot(tt[1:] - tt[0], ego[k, :, 0], label="dx  (sideways)")
    ax.plot(tt[1:] - tt[0], ego[k, :, 1], label="dy  (vertical)"); ax.grid(alpha=.3); ax.legend(fontsize=9); ax.set_xlabel("t (s)")
    ax.set_title("ego-motion translation, previous-frame axes", fontsize=10)
    ax = fig.add_subplot(gs[2, :])
    ax.plot(tt[1:] - tt[0], np.degrees(ego[k, :, 4]), label="ry  (yaw, turning)"); ax.plot(tt[1:] - tt[0], np.degrees(ego[k, :, 3]), label="rx  (pitch)")
    ax.plot(tt[1:] - tt[0], np.degrees(ego[k, :, 5]), label="rz  (roll)")
    for j in show:
        ax.axvline(tt[j] - tt[0], color="0.8", lw=0.8, zorder=0)
    ax.grid(alpha=.3); ax.legend(fontsize=9, ncol=3); ax.set_xlabel("t (s)"); ax.set_ylabel("deg / frame")
    ax.set_title(f"ego-motion rotation per frame (net yaw over the window: {yaw[k]:+.0f}°); grey lines = the frames shown above", fontsize=10)
    fig.suptitle(f"SpatialVID-HQ: one 8 s window at 15 Hz  (val, walk, channel “{row.channel_title}”)", fontsize=12)
